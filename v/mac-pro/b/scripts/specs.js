...
}}})();return b});AC.define("shared/StickyNav",["require"],function(a){var c=function(e,d){d=d||{};
this.element=AC.Element.select(e);this.body=document.body;AC.Element.addClassName(this.body,"stickynav");
this.main=AC.Element.getElementById("main");this.elementToShim=d.shimElement||this.main;
this.shimProperty=d.shimProperty||"padding-top";this.wrapper=document.createElement("div");
...
};var b=c.prototype;b.idleTimerInterval=function(){this.idleTime++;if(!this.isFixed&&(this.currScrollY>(this.dockY+this.navHeight))&&this.idleTime==this.timerOut){AC.Element.addClassName(this.body,"stickynav-idle");
this.setFixed()}};b.resetTimer=function(){this.idleTime=0};b.setupIdleListener=function(){var d=["mousemove","click","keydown","resize"];
d.forEach(function(e){this.bindOnEvent(e,this.resetTimer)}.bind(this))};b.attachScrollListener=function(){this.bindOnEvent("scroll",this.scrollListener)
};b.scrollListener=function(){if(!this.isDocking){if(this.currScrollY>0){var d=this.scrollDirection();
if(d>0){this.onScrollDown()}else{if(d<0){this.onScrollUp()}}if(this.hasHotspots&&this.isFixed){this.updateFromHotspots()
}}else{this.reset()}}this.resetTimer()};b.onScrollDown=function(){if(this.currScrollY>(this.dockY+this.navHeight)){if(this.isFixed){if(this.willHide){if(!this.isInView()){this.reset()
...
};b.onScrollUp=function(){if(this.willHide){if(this.wrapper.offsetTop>=this.currScrollY){this.cancelHide()
}}if(!this.isFixed&&(this.currScrollY>(this.dockY+this.navHeight))&&this.scrollThresholdMet()){this.setFixed()
}};b.shouldDock=function(){return this.isFixed&&(this.currScrollY<=this.dockY)};
b.isAtPageBottom=function(){return this.currScrollY>=document.body.scrollHeight-window.innerHeight
};b.outsideOfBottomThreshold=function(){return this.currScrollY<document.body.scrollHeight-window.innerHeight-this.bottomThreshold
};b.setFixed=function(){var d={};d[this.shimProperty]=this.navHeight+"px";AC.Element.addClassName(this.body,"stickynav-undocked");
AC.Element.setStyle(this.elementToShim,d);this.isFixed=true;this.willHide=false
};b.dockNav=function(){if(this.browserUsesTransitions){this.isDocking=true;AC.Element.addClassName(this.body,"stickynav-docking");
this._boundTransitionEndCallback=this._boundTransitionEndCallback||this.reset.bind(this);
AC.Element.addVendorPrefixEventListener(this.wrapper,"transitionEnd",this._boundTransitionEndCallback)
}else{this.reset()}};b.reset=function(){if(this.isFixed&&!this.currentHotspot){var d={};
d[this.shimProperty]="";AC.Element.removeClassName(this.body,"stickynav-undocked");
AC.Element.removeClassName(this.body,"stickynav-idle");AC.Element.setStyle(this.elementToShim,d);
...
b.initiateHide=function(){AC.Element.setStyle(this.wrapper,{position:"absolute",top:this.currScrollY+"px"});
this.willHide=true};b.cancelHide=function(){AC.Element.setStyle(this.wrapper,{position:"",top:""});
this.willHide=false};b.updateFromHotspots=function(){var d=this.isOverHotspot();
if(d){if(!this.currentHotspot){this.applyHotspotNavStyle(d);this.currentHotspot=d
}}else{if(this.currentHotspot){this.removeHotspotNavStyle(this.currentHotspot);
this.currentHotspot=false}}};b.scrollThresholdMet=function(){var d=false;var e=Math.abs(this.currScrollY-this.scrollThresholdStartPoint);
if(e>=this.scrollThreshold){d=true;this.scrollThresholdStartPoint=this.currScrollY
}return d};b.isInView=function(){return(this.currScrollY-(this.wrapper.offsetTop+this.navHeight))<=0
};b.scrollDirection=function(){var d=0;var e=this.currScrollY-this.lastScrollY;
if(e!==0){d=e>0?1:-1;this.lastScrollY=this.currScrollY}return d};b.getHotspotElements=function(){return AC.Element.selectAll("[data-"+this.dataAttribute+"]")
};b.hotspotElementToObject=function(d){var g=65;var f=AC.Element.cumulativeOffset(d).top;
var e={element:d,top:f-g,bottom:f+d.offsetHeight-g,navstyle:d.getAttribute("data-"+this.dataAttribute)};
return e};b.loadHotspotRegistry=function(){var d=this.getHotspotElements();if(d.length>0){d.forEach(function(e){this.hotspotRegistry.push(this.hotspotElementToObject(e))
}.bind(this));this.hasHotspots=true}};b.isOverHotspot=function(){var d=false;this.hotspotRegistry.some(function(e){if((this.currScrollY>=e.top)&&(this.currScrollY<=e.bottom)){d=e;
return true}}.bind(this));return d};b.applyHotspotNavStyle=function(d){AC.Element.addClassName(this.body,d.navstyle)
};b.removeHotspotNavStyle=function(d){AC.Element.removeClassName(this.body,d.navstyle)
};b.getScrollY=function(){return window.pageYOffset||document.documentElement.scrollTop||document.body.scrollTop
};b.bindOnEvent=function(g,k,e){e=e||this;var d=false;var j=false;var f=false;var i=function(l,m){if(l.type=="scroll"){this.currScrollY=this.getScrollY();
if(this.shouldDock()){this.dockNav()}}if(d===false){j=true;h.apply(this,[l,m])}else{f=true
}};var h=function(l,m){if(j===true){j=false;d=true;m.call(this,l);window.requestAnimationFrame(h.bind(this,l,m))
}else{d=false;if(f){i.apply(e,[l,m]);f=false}}};AC.Element.addEventListener(window,g,function(l){i.apply(e,[l,k])
})};return c});AC.define("eventEmitter/EventEmitter",[],function(){var d=function(f){this.context=f
...
}.bind(this))},this);return this};return d});AC.define("animationSequencer/vendor/utils",[],function(){return{isNum:function(a){return typeof a==="number"
},isArray:function(b){var a=Object.toString;return a.call(b)==="[object Array]"
...
var f=c("shared/Detector");window.DEBUG=false;var g="ontouchstart" in window;return{init:function(){var h=b.sequenceForContainer(a);
var i=AC.Element.select("#specs .specs-desktop .dimensions .callouts");if(h){this.startSequence(h,i)
}else{this.ensureEnded()}},startSequence:function(n,j){var h=new d(n._sequence,[2,3.5],{autoInit:true});
var l=0.1;var k=g?1.9:1.2;var m=g?3.8:3;var i=g?250:50;h.addTime(l,function(){AC.Element.addClassName(j,"show-height")
});h.addTime(k,function(){AC.Element.addClassName(j,"show-diameter")});h.addTime(m,function(){AC.Element.addClassName(j,"show-weight")
});if(typeof n._sequence.addEventListener==="function"){n._sequence.addEventListener("ended",this.ensureEnded)
}else{if(typeof n._sequence.on==="function"){n._sequence.on("ended",this.ensureEnded)
}}n.play()},ensureEnded:function(){var h=AC.Element.select("#specs .specs-desktop .dimensions .callouts");
AC.Element.addClassName(h,"show-height show-diameter show-weight")}}});AC.define("performance/desktop/HeightMargin",["require"],function(a){function c(f,e,d){this._target=f;
...
return e+"px"};return c});AC.define("specs/desktop/main",["require","shared/Detector","shared/StickyNav","specs/shared/DimensionsVideoAnimation","performance/desktop/HeightMargin"],function(a){window.DEBUG=false;
var g=a("shared/Detector");var f=a("shared/StickyNav");var b=AC.Element.select(".imagery .callouts");
AC.Element.addClassName(b,"pre");if(g.isMobile()){return false}var d=a("specs/shared/DimensionsVideoAnimation");
var c=a("performance/desktop/HeightMargin");if(!AC.Environment.Feature.touchAvailable()&&!(AC.Environment.Browser.name=="IE"&&AC.Environment.Browser.version<9)){var e=new f(".productnav-wrapper",{shimElement:AC.Element.select(".specs-desktop")})
}AC.onDOMReady(function(){var i=AC.Element.select(".imagery");var h=new c(b,i,[0,-27,0,0]);
AC.Element.removeClassName(b,"pre");if(AC.Environment.Feature.supportsCanvas()){d.init()
}})});